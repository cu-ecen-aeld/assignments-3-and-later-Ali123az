...
mkdir rootfs
cd rootfs
mkdir -p bin sbin usr/bin usr/sbin usr/lib etc lib lib64 dev proc sys tmp var home
...
for lib in $(${CROSS_COMPILE}readelf -a bin/busybox | grep "Shared library" | sed -E 's/.*\[(.*)\].*/\1/'); do
